...
func (eh *eventServiceHandler) DeleteEventHandler(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	eventName, ok := vars["Event"]
	fmt.Printf("eventName is %s", eventName)
	if !ok {
		w.WriteHeader(400)
		fmt.Fprint(w, `{"error":"No event name found}"`)
		return
	}
	var err error
	//name := strings.ToLower(eventName)
	err = eh.dbhandler.DeleteEventByName(eventName)
	if err != nil {
		fmt.Fprintf(w, `{"error":"%s"}`, err)
		return
	}
	fmt.Fprintf(w, "Event with name %s is deleted from database", eventName)
}
