...
func parsePrimary(lex *lexer) Expr {
	switch lex.token {
	case scanner.Ident: //note in expr like sqrt(A / pi), Ident would be sqrt,A,pi
		id := lex.text()
		//	fmt.Println("id= ", id)
		lex.next() // consume Ident
		//		fmt.Println("an= ", lex.text())
		if lex.token != '(' {
			//	fmt.Println("h = ", id)
			return Var(id)
		}
		...
		if lex.token != ')' {
			for {
				//fmt.Println("appb ", lex.text())
				args = append(args, parseExpr(lex))
				//	fmt.Println("appa ", lex.text())
				if lex.token != ',' {
					//fmt.Println("tok = ", lex.token, lex.text())
					break
				}
				lex.next() // consume ','
			}
